...
public class ParseRDPresults {
	public static final String DIR = "C:\\Users\\kwinglee.cb3614tscr32wlt\\Documents\\Fodor\\JobinCollaboration\\Gemcitabine and Biofilm Reassociation\\RDP results\\";
	
	public static void main(String[] args) throws IOException {
		//get hierarchical file results
		BufferedReader hier = new BufferedReader(new FileReader(new File(DIR + "gemBiof_hierarch_merge.txt")));
		...
		for(String key: out.keySet()) {
			...
			for(int i = 4; i < sp.length; i++) {
				file.write("\t" + 
						sp[i].replace(".fasta", "").replaceAll("_R[12]", "").replace("join", ""));
			}
			//write read
			file.write("\nread");
			for(int i = 4; i < sp.length; i++) {
				if(sp[i].contains("R1")) {
					file.write("\tR1");
				} else if(sp[i].contains("R2")) {
					file.write("\tR2");
				} else {
					file.write("\t");
					System.err.println("Bad read: " + i + " " + sp[i]);
				}
			}
			file.write("\n");
		}
