...
public class WriteRDPScriptsRun2_4483rerun {
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedWriter allWriter = new BufferedWriter(new FileWriter(new File(SCRIPT_FOLDER + "runAllRDPrun2_4483rerun.sh")));

		for(int i = 0; i < files.length; i++) {
			if(files[i].getName().endsWith("join.fastq") && files[i].getName().startsWith("rerun")) {
