/*
 * Trim primers from run2 4483 rerun reads and generate both fastq and fasta
 */
package kw_meyer;
...
public class TrimPrimersRun2_4483rerun {
	private static String DIR = "/nobackup/afodor_research/kwinglee/meyer/";
	private static String INDIR = DIR + "cardia_seq2_fastqs/4483-KAM-P1_R-36382362/";
	...
	private static void filter(File file) throws IOException {
		...
		BufferedWriter fq1 = new BufferedWriter(
				new FileWriter(
						new File(TrimPrimersRun2_4483rerun.OUTDIR + "rerun" + read1.getName().replace(".gz", ""))));
		BufferedWriter fq2 = new BufferedWriter(
				new FileWriter(
						new File(TrimPrimersRun2_4483rerun.OUTDIR + "rerun" + read2.getName().replace(".gz", ""))));
		BufferedWriter fa1 = new BufferedWriter(
				new FileWriter(
						new File(TrimPrimersRun2_4483rerun.OUTDIR + "rerun" + read1.getName().replace("q.gz", "a"))));
		BufferedWriter fa2 = new BufferedWriter(
				new FileWriter(
						new File(TrimPrimersRun2_4483rerun.OUTDIR + "rerun" + read2.getName().replace("q.gz", "a"))));
