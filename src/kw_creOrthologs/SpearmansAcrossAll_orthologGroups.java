...
public class SpearmansAcrossAll_orthologGroups
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter(new File(
			"/nobackup/afodor_research/af_broad/orthologs/orthologGroupSpearmanAcrossAll.txt")));
	...
	private static HashMap<File, List<Float>> getFilesToDo() throws Exception
	{
		...
		for(String s : files)
			if(s.startsWith("orthogroup") && s.endsWith("_dist.txt") && !s.contains("carolina"))
			{
				...
				{
					if(includeSet.contains(f.getName().replace(".fasta_dist.txt", "")))
						map.put(f, getVals(f));
				}
	...
	private static String getIdentifier(File f) throws Exception
	{
		...
		return f.getName().replace(".fasta_dist.txt", "");
	}
}
