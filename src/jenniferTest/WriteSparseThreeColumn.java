...
public class WriteSparseThreeColumn
{
	...
	{
		File topDir = new File( "/projects/afodor_research/jwellerTest/rdpOutDir");
		...
		for( int x=1; x < NewRDPParserFileLine.TAXA_ARRAY.length; x++)
		{
			 BufferedWriter writer = new BufferedWriter(
					 ...
					 "/projects/afodor_research/jwellerTest/spreadsheets/sparseThreeColumn_" + 
		...
		for(String s : topDir.list())
		{
			...
			if( s.endsWith("rdpOut.txt"))
			{
				...
				for( int x=1; x < NewRDPParserFileLine.TAXA_ARRAY.length; x++)
				{
					...
					{
						writer.write( s.replaceAll("rdpOut.txt", "") + "\t" + 
