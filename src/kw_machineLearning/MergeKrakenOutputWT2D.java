/*
 * Merge the kraken outputs for WT2D 
...
public class MergeKrakenOutputWT2D {
	public static String DIR = "/nobackup/afodor_research/kwinglee/machineLearning/wt2d/minikraken/";
	...
	public static void main(String[] args) throws Exception {
		...
		for(String line = m.readLine(); line != null; line = m.readLine()) {
			...
			if(sp.length > 1) {//ignore empty lines
				...
				String group = sp[1];
				if(group.equals("IGT")) {
					group = "impaired_glucose_tolerance";
				} else if(group.equals("NGT")) {//normal glucose tolerance
					group = "n";
				} else if(!group.equals("T2D")) {
					System.out.println("Extra group: " + group);
				}
				metaMap.put(id, group);
			}
		}
		m.close();
		...
		for(int i = 0; i < dataset.length; i++) {
			if(dataset[i].equals("WT2D")) {
				seg.put(sampleID[i].replace("S", ""), disease[i]);
			}
		}
		...
		for(String key : keys) {
			...
			if(seg.containsKey(key)) {
				...
				System.out.println("\tmissingSegata");
			}
		}
		System.out.println();
